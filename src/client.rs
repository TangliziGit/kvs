...
impl<'de> KvsClient<'de> {
    ...
    pub fn connect(addr: &str) -> Result<KvsClient<'de>> {
        ...
        let reader =
            serde_json::de::Deserializer::from_reader(reader).into_iter::<Response>();
    ...
    pub fn set(&mut self, key: String, value: String) -> Result<()> {
        ...
        let next = self.reader.next()
            .ok_or(ErrorKind::UnexpectedError("Can not deserialize next response"))?;
        match next? {
            Response::Set(Ok(_)) => Ok(()),
            Response::Set(Err(err)) =>
                Err(Error::from(ErrorKind::StringError(err))),
            _ =>
                Err(Error::from(ErrorKind::UnexpectedError("Client received an unexpected response"))),
        }
    }
